package com.henrique.nookio_payments.core.audit_logs.scheduler;
...
public class AuditLogRetryScheduler {
    ...
    public void retryLocalAuditLogs() {
        ...
        log.info("[PAYMENTS_AUDIT_LOG_RETRY_STARTED] retrying {} local audit logs", fallbacks.size());
        ...
        try {
            ...
            log.info("[PAYMENTS_AUDIT_LOG_RETRY_SUCCESS] cleared {} local audit logs", fallbacks.size());
        } catch (Exception e) {
            log.warn("[PAYMENTS_AUDIT_LOG_RETRY_FAILED] analytics-api still unreachable: {}", e.getMessage());
        }
    }
}
